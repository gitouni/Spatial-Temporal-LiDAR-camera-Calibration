...
def options():
    ...
    arg_parser.add_argument("--range_x",type=float,nargs=7, default=[0.2,0.2,0.2,0.5,0.5,0.5,1.5])
    arg_parser.add_argument("--particle_size",type=int,default=4000)
    arg_parser.add_argument("--max_iter",type=int,default=200)
...
if __name__ == "__main__":
    ...
    init_se3_mat = np.eye(4)
    init_se3_mat[:3,:] = init_se3_data.reshape(3,4)
    scale = init_sim3_data[-1]
    rotvec, tsl = toVec(init_se3_mat)
    init_sim3_log[:3] = rotvec
    init_sim3_log[3:6] = tsl 
    init_sim3_log[-1] = scale
    gt_sim3_data = np.loadtxt(args.gt_sim3)
    gt_se3_data = gt_sim3_data[:12]
    gt_sim3_log = np.zeros(7)
    gt_se3_mat = np.eye(4)
    gt_se3_mat[:3,:] = gt_se3_data.reshape(3,4)
    scale = gt_sim3_data[-1]
    rotvec, tsl = toVec(gt_se3_mat)
    gt_sim3_log[:3] = rotvec
    gt_sim3_log[3:6] = tsl 
    gt_sim3_log[-1] = scale
    print("init_sim3:{}".format(init_sim3_log))
    print("gt_sim3:{}".format(gt_sim3_log))    
    ...
    src_img = np.array(dataStruct.get_cam0(0))
    tgt_img = np.array(dataStruct.get_cam0(3))
