...
class FPS:
    def __init__(self, pcd_xyz:np.ndarray, n_samples:int):
        ...
        self.selected_pts_id = np.zeros(n_samples,dtype=np.int32)
        ...
        self.selected_pts_id[0] = self.start_idx
        self.n_selected_pts = 1

    def get_selected_pts(self, return_index=False):
        self.selected_pts = np.squeeze(self.selected_pts_expanded, axis=1)
        if not return_index:
            return self.selected_pts
        else:
            return self.selected_pts, self.selected_pts_id
    ...
    def step(self):
        if self.n_selected_pts < self.n_samples:
            ...
            self.selected_pts_id[self.n_selected_pts] = res_selected_idx
    ...
    def fit(self, return_index=False):
        for _ in range(1, self.n_samples):
            self.step()
        return self.get_selected_pts(return_index)
