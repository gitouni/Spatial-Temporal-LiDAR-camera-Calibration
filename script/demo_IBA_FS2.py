...
if __name__ == "__main__":
    ...
    src_orb_pose = src_fs.getNode("Pose").mat()
    tgt_orb_pose = tgt_fs.getNode("Pose").mat()
    camera_motion:np.ndarray = inv_pose(tgt_pose) @ src_pose  # Tc2w * Twc1
    src_matched_pts, _ = npproj(mappoints, src_orb_pose, intran, src_img.shape[:2])
    tgt_matched_pts, _ = npproj(mappoints, tgt_orb_pose, intran, tgt_img.shape[:2])
