...
/**
 * @brief Hand-eye calibration (from B to A) used in degenerated scene
 * 
 * @param TaList Motions of Sensor A
 * @param TbList Motions of Sensor B
 * @return Eigen::Matrix4d TAB (transformation from B to A)
 */
std::tuple<Eigen::Matrix3d, Eigen::Vector3d, double> DGHECalib(const std::vector<Eigen::Isometry3d> &TaList, const std::vector<Eigen::Isometry3d> &TbList, const double &dg_threshold=0.01){
    assert(TaList.size()==TbList.size());
    std::vector<Eigen::Vector3d> alphaList, betaList;
    Eigen::MatrixX4d A(3*TaList.size(), 4);  // 3N, 4
    Eigen::VectorXd b(3*TaList.size());  // 3N
    ...
    alpha_mean.setZero();
    beta_mean.setZero();
    std::vector<double> tAnorm, tBnorm;
    for(std::size_t i = 0; i < TaList.size(); ++i){
        // Ta
        Eigen::AngleAxisd ax;
        ax.fromRotationMatrix(TaList[i].rotation());
        Eigen::Vector3d alpha = ax.angle() * ax.axis();
        alphaList.push_back(alpha);
        if(alpha.norm() < dg_threshold)
        {
            tAnorm.push_back(TaList[i].translation().norm());
            tBnorm.push_back(TbList[i].translation().norm());
        }
        // Tb
        alpha_mean += alpha;
        ax.fromRotationMatrix(TbList[i].rotation());
    ...
    // Do NOT Compute Translation Part tAB
    Eigen::Vector3d tAB = Eigen::Vector3d::Zero();
    double scale;
    double num = 0, den = 0;
    for(std::size_t i = 0; i < tAnorm.size(); ++i)
    {
        num += tAnorm[i] * tBnorm[i];
        den += tAnorm[i] * tAnorm[i];
    }
    std::printf("Toatal Frames: %ld, Degenerated Frames: %ld",TaList.size(), tAnorm.size());
    return std::make_tuple(RAB, tAB, num / den);
