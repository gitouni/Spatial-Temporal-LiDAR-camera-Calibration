...
{
    CurvatureParams curve_params;
    curve_params.max_pts = iba_params.neigh_max_pts;
    curve_params.min_pts = iba_params.neigh_min_pts;
    int cnt = 0;
    int edge_cnt = 0;
    ...
    for(std::size_t Fi = 0; Fi < PointClouds.size(); ++Fi)
    {
        ...
        for(std::size_t sub_idx = 0; sub_idx < subindices.size(); ++ sub_idx)
        {
            ...
            VecVector3d neighbor_pts = SelectPoints(PointClouds[Fi], neigh_idx);
            if(neighbor_pts.size() < iba_params.neigh_min_pts)
                continue;
            ...
            ceres::LossFunction *loss_function_2d = new ceres::HuberLoss(iba_params.robust_kernel_delta);
            ceres::CostFunction *cost_function_2d = IBA_GPRFactor::Create(
                iba_params.init_sigma, iba_params.init_l, iba_params.sigma_noise, neighbor_pts,
                initSE3_4x4, pKF->mnMaxY, pKF->mnMaxX, pKF->fx, pKF->fy, pKF->cx, pKF->cy, u0, v0,
                u1_list, v1_list, R_list, t_list, iba_params.optimize_gpr, false
            );
            problem.AddResidualBlock(cost_function_2d, loss_function_2d, params);
            Eigen::Vector3d pd1, pd2, pdcross, nn_pt3d;
            Eigen::Matrix3d base;
            double pv1, pv2;
            auto ret = ComputeCurvature(neighbor_pts, MapPoint * init_scale, curve_params);
            if(ret.has_value())
            {
                std::tie(pd1, pd2, nn_pt3d, pdcross, pv1, pv2) = ret.value();
                base.row(0) = pd1;
                base.row(1) = pd2;
                base.row(2) = pdcross;
                ceres::LossFunction *loss_function_3d = new ceres::HuberLoss(iba_params.robust_kernel_3ddelta);
                ceres::CostFunction *cost_function_3d = Quadratic_AlignFactor::Create(
                    MapPoint, base, 1.0/abs(pv1), 1.0/abs(pv2), nn_pt3d
                );
                problem.AddResidualBlock(cost_function_3d, loss_function_3d, params);
            }
            #pragma omp critical
            {
        ...
        #pragma omp critical
        {
            ...
            if(iba_params.verborse && (cnt % 100 == 0))
                std::printf("Problem Building: %0.2lf %% finished (%d iters | %d factors)\n", (100.0*cnt)/PointClouds.size(),cnt, edge_cnt);
        }
    }
    assert(edge_cnt>0);
...
int main(int argc, char** argv){
    ...
    iba_params.neigh_min_pts = runtime_config["neigh_min_pts"].as<int>();
