...
std::vector<ORB_SLAM2::KeyFrame*> findEnoughRotationConvis(const ORB_SLAM2::KeyFrame* pKF, const std::vector<ORB_SLAM2::KeyFrame*> &convisKFs, const double &rot_threshold)
{
    std::vector<ORB_SLAM2::KeyFrame*> validConvisKFs;
    cv::Mat refPose = pKF->GetPoseInverseSafe();
    for(auto const pConvF:convisKFs)
    {
        Eigen::Matrix4d relPose;
        cv::cv2eigen(refPose * pConvF->GetPoseSafe(), relPose);
        Eigen::Vector3d rotvec = SO3Log(relPose.topLeftCorner(3,3));
        if(rotvec.norm() >= rot_threshold)
        {
            validConvisKFs.push_back(pConvF);
        }
    }
    return validConvisKFs;
}
 ...
 const bool &multiprocessing=false, const bool &verborse=false)
{
    ...
    for(std::size_t Fi = 0; Fi < PointClouds.size(); ++Fi)
    {
        ...
        std::vector<ORB_SLAM2::KeyFrame*> RawConvisKeyFrames, ConvisKeyFrames;
        RawConvisKeyFrames = pKF->GetCovisiblesByWeightSafe(iba_params.min_covis_weight);  
        ConvisKeyFrames = findEnoughRotationConvis(pKF, RawConvisKeyFrames, iba_params.min_rot_diff);
...
int main(int argc, char** argv){
    ...
    iba_params.min_rot_diff = runtime_config["min_rot_diff"].as<double>();
