    ...
    const int leaf_size, const double max_corr_dist, CorrSet &corrset)
{
    ...
    for (std::size_t i = 0; i < points.size(); ++i)
    {
        ...
        if(point.z() > 0){
            ...
            if (0 <= intu && intu < W && 0 <= intv && intv < H){
 ...
 * @param KdTrees 
 * @param vTwl 
 * @param KFIdMap mnId -> FileId 
 * @param KeyFrames 
 * @param iba_params 
 * @param multiprocessing 
 * @param verborse 
 * @return std::tuple<double, double, double, int, int>  fobj, C, valid_edge_cnt, edge_cnt
 */
 ...
 const bool &multiprocessing=false, const bool &verborse=false)
{
    ...
    Eigen::Isometry3d Tlc = Tcl.inverse();
    ...
    for(std::size_t Fi = 0; Fi < PointClouds.size(); ++Fi)
    {
        ...
        Eigen::Matrix4d Tcw;  // Tcw with real size
        ...
        if(iba_params.num_best_convis > 0)
            ConvisKeyFrames = pKF->GetBestCovisibilityKeyFramesSafe(iba_params.num_best_convis);
        else
        ...
        relCVPoseList.reserve(ConvisKeyFrames.size());
        ...
        for(auto &corr:corrset)
        {
            ...
            cv::cv2eigen(mapKpt2Mpt[point2d_idx]->GetWorldPos(), MapPoint);  // sPw
            Eigen::Vector3d MapRefPoseRS = Tcw.topLeftCorner(3,3) * MapPoint + Tcw.topRightCorner(3,1); // Pci 
            MapRefPoseRS *= scale;  // with scale
            bool is_planefit;
            double dist;
            std::tie(is_planefit, dist) = ComputeAlignmentDist(KdTrees[Fi].get(), PL, Tlc * MapRefPoseRS,
            ...
            for(std::size_t pKFConvi = 0; pKFConvi < ConvisKeyFrames.size(); ++pKFConvi){
                ...
                Eigen::Matrix4d relCVPose = relCVPoseList[pKFConvi];  // Twc2 * inv(Twc1) with scale
                ...
                Eigen::Vector3d MapConvisPose = relCVPose.topLeftCorner(3, 3) * MapRefPoseRS + relCVPose.topRightCorner(3, 1);
                assert(KFIdMap.count(pKFConv->mnId) > 0);
                const int ConvisFileId = KFIdMap.at(pKFConv->mnId);
                std::tie(is_planefit, dist) = ComputeAlignmentDist(KdTrees[ConvisFileId].get(), PointClouds[ConvisFileId], Tlc * MapConvisPose,
                    iba_params.norm_max_pts, iba_params.norm_radius, iba_params.norm_reg_threshold, iba_params.min_diff_dist, iba_params.use_plane);
                #pragma omp critical
                {
                    if(dist < iba_params.corr_3d_3d_threshold)
