...
bool SurfNormAnalyticCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {
            ...
            (dp_by_se3.block<3,3>(0, 3)).setIdentity();
            Eigen::Map<Eigen::Matrix<double, 1, 7, Eigen::RowMajor> > J_se3(jacobians[0]);  // jacobian size: [1,7]
            J_se3.setZero();
...
void getTransformFromSe3(const Eigen::Matrix<double,6,1>& se3, Eigen::Quaterniond& q, Eigen::Vector3d& t){
    ...
    if(theta<1e-10)
    {
        ...
        imag_factor = 0.5-0.0208333*theta_sq+0.000260417*theta_po4;  // Tayor Approximation
    }
    else
    {
