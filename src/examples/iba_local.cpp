    ...
    const std::vector<ORB_SLAM2::KeyFrame*> &KeyFrames, double* params, ceres::Problem &problem, const IBAGPR3dParams &iba_params)
{
    ...
    for(std::size_t Fi = 0; Fi < PointClouds.size(); ++Fi)
    {
        ...
        CorrSet pt2d3d_map = FindConvisProjCorr(points, pKF, pConvisKFs, KptMapList, relPoseList, iba_params.kdtree2d_max_leaf_size, iba_params.max_pixel_dist);
        ...
        for(auto const &[point2d_idx, point3d_idx]:pt2d3d_map)
        {
            // Should not happen
