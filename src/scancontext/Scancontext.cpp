...
Eigen::MatrixXd SCManager::makeScancontext(const std::vector<Eigen::Vector3d> & _scan_down ){
    TicTocV2 t_making_desc;
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        Eigen::Vector3d pt = _scan_down[pt_idx];
        pt(2) += LIDAR_HEIGHT;

        // xyz to ring, sector
        azim_range = pt.norm();
        azim_angle = xy2theta(pt(0), pt(1));
        ...
        // taking maximum z 
        if ( desc(ring_idx-1, sctor_idx-1) < pt(2) ) // -1 means cpp starts from 0
            desc(ring_idx-1, sctor_idx-1) = pt(2); // update for taking maximum value at that bin
    }
