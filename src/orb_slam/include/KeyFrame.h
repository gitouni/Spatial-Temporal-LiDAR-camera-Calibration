...
class KeyFrameConstInfo{
public:
    KeyFrameConstInfo(cv::FileStorage &fs, KeyFrameDatabase *pKFDB, ORBVocabulary *pVoc);
    static bool lId(KeyFrameConstInfo* pKF1, KeyFrameConstInfo* pKF2){
...
public:
    ...
    KeyFrame(boost::archive::binary_iarchive &ora, const KeyFrameConstInfo* info, Map *pMap, vector<MapPoint*> Mpts ,unordered_map<int, MapPoint*> mapMptsId);
...
public:
    ...
    const int mnMaxY;
    const cv::Mat mK;
    std::unordered_map<MapPoint*, int> mmapMpt2Kpt;
