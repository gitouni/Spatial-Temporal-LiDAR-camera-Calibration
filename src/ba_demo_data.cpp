...
int main(int argc, char **argv) {
  ...
  for (DMatch m:matches) {
    ...
    if (d == 0)   // bad depth
      continue;
    float dd = d / 5000.0; // scale factor: 5000
...
void find_feature_matches(const Mat &img_1, const Mat &img_2,
  ...
  if ( descriptors_1.empty() )
    error(0,"No descriptors in Img1",CV_Func,__FILE__,__LINE__);    
  if ( descriptors_2.empty() )
    error(0,"No descriptors in Img1",CV_Func,__FILE__,__LINE__);
  //-- 第三步:对两幅图像中的BRIEF描述子进行匹配，使用 Hamming 距离
  vector<DMatch> match;
  // BFMatcher matcher ( NORM_HAMMING );
  matcher->train();  // for FLANN Matcher
...
void bundleAdjustmentGaussNewton(
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    for (std::size_t i = 0; i < points_3d.size(); i++) {
      ...
      cost += e.squaredNorm();
      Eigen::Matrix<double, 2, 6> J;  // [omega, epsilon]
...
public:
  ...
  virtual void computeError() override {
    ...
    pos_pixel /= pos_pixel[2];   // Project to Normalization Plane
  ...
  virtual void linearizeOplus() override {
    ...
    double Z2 = Z * Z;
    _jacobianOplusXi
      << fx * X * Y / Z2, -fx - fx * X * X / Z2, fx * Y / Z, -fx / Z, 0, fx * X / Z2, 
      fy + fy * Y * Y / Z2, -fy * X * Y / Z2, -fy * X / Z, 0, -fy / Z, fy * Y / (Z * Z);
  }
